...
#' @export
labVolumesTableMI <- function(StormName,StormStart,StormEnd,tableOut,bottlePickup){
  ...
  for (i in 1:length(StormName)) {
    ...
    for (j in 1:nrow(labTable)) {
      cat("\t",paste0(labTable$subNum[j],paste0(rep(" ",max(nchar(labTable$subNum))),collapse=""),collapse=""),"\t",strftime(labTable$datetime[j],format="%Y-%m-%d %H:%M",tz="UTC"),"\t",labTable$mL[j],"\n")
    }
    cat("==================================================================================","\n")
